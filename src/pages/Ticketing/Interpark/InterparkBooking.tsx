...
import { Box, HStack, Text, Badge, VStack, Heading, Grid, Button, Image, useToast } from "@chakra-ui/react";
...
const generateInitialSeatsForSection = (
  ...
  if (isHard) {
    baseOccupancyPercent = 0.65 + delaySec * 0.25;
  } else {
...
const InterparkBooking = () => {
  ...
  useEffect(() => {
    ...
    const triggerNext = () => {
      const nextDelay = Math.random() * 1200 + 800; // 0.8s to 2s
      return setTimeout(() => {
        spawnDistraction();
        timerId = triggerNext();
      }, nextDelay);
        ...
        <HStack justify="space-between">
          ...
          <HStack spacing={3}>
            <Badge colorScheme={mode === "jaehyun" ? "purple" : mode === "nboom" ? "red" : "blue"} variant="solid" px={2} py={0.5} rounded="md">
              {mode === "jaehyun" ? "재현 모드" : mode === "nboom" ? "엔붐온 모드" : "일반 모드"}
            </Badge>
            <Box
              as="button"
              ...
              >
                ...
                <Box bg="rgba(0,0,0,0.5)" w="full" p={4} borderTop="1px solid" borderColor="purple.800">
                  <Button
                    ...
                    _active={{ bgGradient: "linear(to-r, purple.700, indigo.800)" }}
                    w="full"
                    onClick={() => {
                      setActiveFullScreenDistraction(null);
                      setPhase("captcha");
